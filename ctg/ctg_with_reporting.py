...
class NumpyDataset(Dataset):
    ...
    def __init__(self, input_file_name, output_file_name, is_train=True):
        ...
        X_train, X_test = train_test_split(X_in, test_size=.10, random_state=1)
...
valid_dataset = NumpyDataset('train_input.csv', 'train_output.csv',
                             is_train=False)
testloader = DataLoader(valid_dataset, batch_size=len(valid_dataset))
    ...
    for epoch in range(num_epochs):  # no. of epochs
        ...
        with torch.no_grad():
            for i, data in enumerate(testloader):
                ...
                dev_loss = criterion(net(inputs), labels)

            dev_losses.append(dev_loss)
            print('[Epoch %d] Dev loss: %.3f' %
                  (epoch + 1, dev_loss))
            if dev_loss < lowest_loss:
                print('Saving model')
                torch.save(net.module.state_dict(), 'example_best_val.pt')

    print('Done Training')
    return train_losses, dev_losses, net
    ...
    with torch.no_grad():
        for data in testloader:
            inputs, labels = data[0].to(device, non_blocking=True) - 1, data[
                1].to(device, non_blocking=True)
            labels = labels.unsqueeze(1)
            batch_loss = loss(net(inputs), labels)
            i += 1
            total += batch_loss
            # total += labels.size(0)
            # correct += (torch.round(
            #     outputs.data.reshape(-1)) == labels).sum().item()
    print('Avg MSE of the network on test set: %0.3f' % (
                total / i))



#net.load_state_dict(torch.load('ctg_2cube_model6.pt'))
# print("validation set accuracy:")
train_losses, dev_losses, net = train(net, trainloader, testloader)

import matplotlib.pyplot as plt

plt.plot(range(len(train_losses)), train_losses, label='Train Loss')
plt.plot(range(len(dev_losses)), dev_losses, label='Test Loss')
plt.legend()
#pdb.set_trace()
#plt.show()
plt.savefig('example_loss_plot.png')
